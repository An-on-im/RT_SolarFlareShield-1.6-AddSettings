...
namespace RT_SolarFlareShield
{
    ...
    public class CompRTSolarFlareShield : ThingComp
    {
        ...
        public override void PostDeSpawn(Map map, DestroyMode _)
        {
        ...
        private void SolarFlareShieldTick(int tickAmount)
        {
            if ((Find.TickManager.TicksGame) % tickAmount == 0)
            {
                if (Active)
                {
                    ...
                    if (map != null)
                    {
                        var settings = LoadedModManager.GetMod<Mod>().GetSettings<Settings>();
                        if (Find.World.GameConditionManager.ElectricityDisabled(map) || parent.Map.GameConditionManager.ElectricityDisabled(map))
                        {
                            compPowerTrader.PowerOutput = -settings.activePowerDraw;
                            rotatorAngle += settings.rotatorSpeedActive * tickAmount;
                            GenTemperature.PushHeat(parent.Position, parent.Map, settings.heatingPerTick * tickAmount);
                        ...
                        {
                            compPowerTrader.PowerOutput = -settings.idlePowerDraw;
                            rotatorAngle += settings.rotatorSpeedIdle * tickAmount;
                        }
                    }
                }
            }
